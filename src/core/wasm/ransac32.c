 ...
 * @param reprojectionError given in pixels, used to separate inliers from outliers of a particular model (e.g., 1 pixel)
 * @returns result
 */
WASM_EXPORT const Mat32* Mat32_pransac_homography(const Mat32* result, const Mat32* mask, const Mat32* src, const Mat32* dest, int numberOfHypotheses, int bundleSize, float reprojectionError)
{
    ...
    for(int h = 0; h < m; h++) {
        ...
        // allocate hypothesis
        hypothesis[h].homography = Mat32_zeros(3, 3);
    ...
    if(numberOfValidHypotheses == 0) {
        ...
        for(int c = 0; c < n; c++) {
            ...
            for(int h = 0; h < m; h++) {
                const Mat32* hom = hypothesis[h].homography;
        ...
        const Mat32* hom = hypothesis[k].homography;
        ...
        // compute a new homography via normalized DLT using only the inliers
        if(numberOfInliers >= 4) {
            ...
            // should we recompute the inliers mask?
            // it depends... why? what is its meaning?
